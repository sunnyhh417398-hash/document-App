...
function seedUsers() {
  // 僅在沒有任何使用者（檔案不存在、無法解析或為空陣列）時才重建預設帳號
...
function listUser(u) {
  return {
    ...
    roleLabel: ROLES[u.role], email: u.email || '', active: u.active !== false, createdAt: u.createdAt,
  };
}

// 取得指定角色且啟用中的使用者
function usersByRole(role) {
  return (loadUsers() || []).filter((u) => u.role === role && u.active !== false);
}
...
// 從請求取得工作階段 token：優先用 Authorization 標頭（桌面/Electron 最可靠），
// 其次 X-Session-Token，最後才是 Cookie。
...
module.exports = {
  ...
  usernameExists, activeAdminCount, publicUser, listUser, usersByRole,
  createSession, getSession, destroySession, destroyUserSessions, parseCookies, tokenFromReq, currentUser,
};
